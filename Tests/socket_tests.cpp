...
constexpr auto TEST_PORT_1 = 9999;
constexpr auto TEST_PORT_2 = 8888;
constexpr auto TEST_PORT_3 = 5555;
...
TEST(try_connect_to_closed_peer, BasicAssertions)
{
  // Create a listening socket
  Socket sock1 = Socket(Socket::Protocol::TCP);
  auto connect_result = sock1.connect(TEST_IP_ADDR,TEST_PORT_1);
...
TEST(partial_recv, BasicAssertions)
{
  // Create a listening socket
  Socket sock1 = Socket(Socket::Protocol::TCP);
  sock1.bind(TEST_PORT_3);
  sock1.listen(1);
  // Connect to listening socket
  Socket sock2 = Socket(Socket::Protocol::TCP);
  sock2.connect(TEST_IP_ADDR, TEST_PORT_3);
  // Accept conneciton
  auto sock3 = sock1.accept();
