...
#endif
{
  ...
  if (m_socket == SOCKET_ERROR_CODE ||
      setsockopt(m_socket, SOL_SOCKET, SO_REUSEADDR, &ENABLE_REUSE,
                 sizeof(int)) < 0) {
#ifdef _WIN32
    WSACleanup();
#endif
...
Socket::Socket(SOCKET socket, Protocol protocol)
    : m_protocol(protocol), m_socket(std::move(socket)) {
#ifdef _WIN32
  if (WSAStartup(MAKEWORD(2, 2), &m_wsaData) != 0) {
    throw std::runtime_error(std::strerror(errno));
  }
...
Socket Socket::accept() {
  ...
  SOCKET client_socket =
      ::accept(m_socket, (sockaddr *)&client_addr, &client_addr_size);
